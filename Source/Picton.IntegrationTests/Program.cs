...
namespace Picton.IntegrationTests
{
	class Program
	{
		...
		private static async Task RunCloudBlobExtensionsTests(string connectionString, string containerName, CancellationToken cancellationToken)
		{
			...
			if (!exists1) await blob1.CreateAsync(null, false, cancellationToken).ConfigureAwait(false);
			...
			if (!exists2) await blob2.CreateAsync(null, false, cancellationToken).ConfigureAwait(false);
			...
			if (!exists3) await blob3.CreateAsync(null, false, cancellationToken).ConfigureAwait(false);
			...
			if (!exists4) await blob4.CreateAsync(null, false, cancellationToken).ConfigureAwait(false);
