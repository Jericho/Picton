...
namespace Picton.IntegrationTests
{
	class Program
	{
		static void Main()
		{
			...
			RunCloudBlobExtensionsTests(connectionString, containerName, cancellationToken).Wait();

			Console.WriteLine("Running blob manager tests...");
			RunBlobManagerTests(connectionString, containerName, cancellationToken).Wait();

			Console.WriteLine("Running queue manager tests...");
			RunQueueManagerTests(connectionString, queueName, cancellationToken).Wait();
		...
		private static async Task RunCloudBlobExtensionsTests(string connectionString, string containerName, CancellationToken cancellationToken)
		{
			...
			// BlobClient
			var blob1 = container.GetBlobClient("test1.txt");
			var exists1 = await blob1.ExistsAsync(cancellationToken).ConfigureAwait(false);
			await blob1.CreateIfNotExistsAsync(cancellationToken).ConfigureAwait(false);

			var leaseId1 = await blob1.TryAcquireLeaseAsync(TimeSpan.FromSeconds(30), 3, cancellationToken).ConfigureAwait(false);
			...
			// PageBlobClient
			var blob2 = new PageBlobClient(connectionString, containerName, "test2.txt");
			var exists2 = await blob2.ExistsAsync(cancellationToken).ConfigureAwait(false);
			await blob2.CreateIfNotExistsAsync(PageBlobClient.PageBlobPageBytes * 10, cancellationToken).ConfigureAwait(false);

			var leaseId2 = await blob2.TryAcquireLeaseAsync(TimeSpan.FromSeconds(30), 3, cancellationToken).ConfigureAwait(false);
			await blob2.UploadTextAsync(new string('A', 525), leaseId2, cancellationToken).ConfigureAwait(false);
			await blob2.UploadTextAsync(new string('B', PageBlobClient.PageBlobPageBytes), leaseId2, cancellationToken).ConfigureAwait(false);
			await blob2.AppendTextAsync(new string('C', PageBlobClient.PageBlobPageBytes), leaseId2, cancellationToken).ConfigureAwait(false);
			...
			var content2b = await blob1.DownloadByteArrayAsync(cancellationToken).ConfigureAwait(false);
			Console.WriteLine(content2a);


			//===========================================================================================
			// Unfortunately, the emulator does not support AppendBlob
			//var blob3 = new AppendBlobClient(connectionString, containerName, "test3.txt");
			//var exists3 = await blob3.ExistsAsync(cancellationToken).ConfigureAwait(false);
			//await blob3.CreateIfNotExistsAsync(cancellationToken).ConfigureAwait(false);

			//var leaseId3 = await blob3.TryAcquireLeaseAsync(TimeSpan.FromSeconds(30), 3, cancellationToken).ConfigureAwait(false);
			//await blob3.UploadTextAsync("Hello World", leaseId3, cancellationToken).ConfigureAwait(false);
			//await blob3.UploadTextAsync("qwerty", leaseId3, cancellationToken).ConfigureAwait(false);
			//await blob3.AppendTextAsync("azerty", leaseId3, cancellationToken).ConfigureAwait(false);
			...
			//var content3b = await blob1.DownloadByteArrayAsync(cancellationToken).ConfigureAwait(false);
			//Console.WriteLine(content3a);
			//===========================================================================================
		}
